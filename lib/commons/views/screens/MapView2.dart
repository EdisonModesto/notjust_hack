...
class MapView2 extends ConsumerStatefulWidget {
  MapView2({super.key, required this.location});

  GeoPoint location;

  static const String routePath = '/map2';

  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _MapView2State();
}

class _MapView2State extends ConsumerState<MapView2> {
        ...
        data: (data) {
          ...
          return events.when(
            data: (eventData) {
              ...
              for (var events in eventData) {
                markers2.add(
                  Marker(
                    point: latLng.LatLng(events.location!.latitude, events.location!.longitude),
                    height: 70,
                    width: 100,
                    child: GestureDetector(
                      onTap: () {
                        mapController.move(
                          latLng.LatLng(
                            events.location!.latitude,
                            events.location!.longitude,
                          ),
                          19,
                        );
                      },
              ...
              return FlutterMap(
                options: MapOptions(
                  initialZoom: 19,
                  initialCenter: latLng.LatLng(widget.location.latitude, widget.location.longitude),
